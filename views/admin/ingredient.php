...
<section id="adminIngredients" class="col-xs-12">
    ...
    <?php if($UserType == 2){ ?>
        ...
        <h3 style="margin-bottom: 30px;">Edit ingredients</h3>
        <?php $error = Session::get('EditError');
        $good = Session::get('EditApply');
        $EditName = Session::get('EditIngredientsName');
        Session::uset('EditIngredientsName');
        ...
        <form action="" id="EditForm" method="POST" name="EditForm" class="col-xs-12 col-sm-6 col-sm-offset-3">
            <div class="form-group <?php if(isset($error) and (strpos($error, 'Title') !== false)){ echo "has-error"; } ?>">
                ...
                <input type="text" id="IngredientsName" name="ingredientsname" class="form-control" required="" value="<?php if(isset($EditName) and $EditName != '' and $EditName != NULL){ echo $EditName; } else { echo $Ingredient['name']; } ?>" />
            </div>
